...
def get_questions(message):
    ...
    if category == "auth":
        return (
        "Пользователь не может войти в рабочий аккаунт СБЕРТЕХ "
        "на корпоративном компьютере после смены пароля. "
        "Проблема возникла сегодня. "
        "При входе появляется сообщение «Неверный пароль»."
        )
...
def make_solution(message, answers):
    ...
    if category == "auth":
        return [
        "Завершите активные сессии в настройках безопасности.",
        "Подождите несколько минут.",
        "Войдите с новым паролем.",
        "Если ошибка сохранится — сбросьте кэш браузера."
        ]
